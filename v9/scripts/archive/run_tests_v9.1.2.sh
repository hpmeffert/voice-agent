...
LOG_DIR="output"
LOG_FILE="$LOG_DIR/test-log-v9.1.2.txt"
mkdir -p "$LOG_DIR"
exec >"$LOG_FILE" 2>&1

echo "# V9.1.2 Test Log"
echo "date_utc: $(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "version_target: v9.1.2"
...
CONFIG_JSON=$(curl -sS --max-time 10 "http://localhost:8085/api/config?user_id=test-user-v912")
echo "$CONFIG_JSON"
...
echo
echo "## Agent UI v9.1.2 checks"
python3 - <<'PY'
from pathlib import Path
import re
text = Path('v9/web-agent/index.html').read_text(encoding='utf-8')
checks = [
    (r'Voice Agent Agent UI V9\.1\.2', 'version label'),
    (r'id="autoRefresh"', 'auto refresh toggle'),
    (r'Auto-Refresh: an \(5s\)', 'auto refresh status label'),
    (r'id="cleanDisplay"', 'display cleanup toggle'),
    (r'Anzeige bereinigen \(Sonderzeichen ausblenden\)', 'display cleanup wording'),
]
for pattern, label in checks:
    assert re.search(pattern, text), f'missing {label}'
print('agent_ui_v912_checks: PASS')
PY
